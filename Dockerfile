...
RUN --mount=type=cache,target=/var/cache/apt/ \
  --mount=type=cache,target=/var/lib/apt/ \
  apt-get update && \
  apt-get install --no-install-recommends --yes build-essential software-properties-common curl git gnupg ssh sudo vim zsh zsh-antigen lsb-release exa nano && \
