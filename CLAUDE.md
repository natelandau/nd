...
- `nd volume register` - discovers volume specs from `[volumes] directories`, fetches all cluster nodes and existing registrations concurrently, plans a register-or-skip action for each spec on each ready node (using the `nfsStorageRoot` node-meta key plus the spec's `relative_path` to build the `HostPath`), and calls `client.volumes.register()` for each new registration. With `--dry-run / -n` it reports what would be registered without touching the cluster.
- `nd volume delete` - discovers volume specs, lists registered host volumes, and calls `client.volumes.delete()` for every registered volume whose name matches a discovered spec. With `--dry-run / -n` it reports what would be deleted.
- `nd volume list` - discovers specs and lists registered host volumes, then renders a joined table showing each spec and which nodes it is registered on.
