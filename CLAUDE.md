...
- A root `typer.Typer()` registers subcommands via `app.add_typer()` (`status`, `stop`).
...
Follow the existing `status` and `stop` commands (`src/nd/commands/`) as references:
